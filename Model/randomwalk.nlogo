...
to setup-landscape
  ...
  ask patches [
    ...
    if (road-value = 0 and I-am-out-of-bounds? = FALSE) [set pcolor black set I-am-out-of-bounds? TRUE] ; rectifies the last column on the right
  ...
  ask patches [
    if (I-am-out-of-bounds? = TRUE) [set watershed-dens 999 set watershed-id 999 set road-value 999]
  ]
...
to setup-agents
  ...
  foreach gis:feature-list-of watershed-dataset [ vector-feature ->
      ...
      create-turtles 1
        [ set xcor item 0 centroid ; if you want turtle to be created at center of vector-feature, also use to center watershed-home-id and watershed-home-dens
  ...
  ask turtle 7  [set xcor 1134 set ycor 1160]; ditto, centroid is outside the boundaries, so move inside
  ask turtles
  [
    let centroid-patch [watershed-id] of patch xcor ycor
    set watershed-home-id [watershed-id] of patch xcor ycor
    ...
    ;OR
    ;move-to one-of patches with [watershed-id = [watershed-home-id] of myself]; myself because patch is asking a turtle, A patch can't access a turtle variable without specifying which turtle.

    ; if random patch in watershed-id of turtles is not the same as watershed-home-id of turtle, then move to centroid-patch
...
to watershed-random [a]
  while [(patch-ahead 1 = nobody) or ([watershed-id] of patch-ahead 1 != a) or ([watershed-id] of patch-ahead 1 = nobody)]
  [
    right ((random 4) * 90)
  ]
end
...
;    forward 10
;  ]
  while [([I-am-road?] of patch-here) and ([watershed-id] of patch-here = watershed-home-id)] ; only cross road if watershed-id is the same as watershed-home-id
  [
...
to-report isNaN [z]
  report not ( z > 0 or z < 0 or z = 0 )
    ...
    ;foreach gis:feature-list-of watershed-dataset [ vector-feature -> let x gis:location-of gis:centroid-of vector-feature] show
@#$#@#$#@
GRAPHICS-WINDOW
294
10
...
@#$#@#$#@
<experiments>
  <experiment name="Road Crossing Rate" repetitions="1000" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup-agents</setup>
    <go>random-walk</go>
    <metric>ticks</metric>
    <metric>[who] of turtles</metric>
    <metric>[watershed-home-id] of turtles</metric>
    <metric>[watershed-home-dens] of turtles</metric>
    <metric>[road-freq] of turtles</metric>
    <enumeratedValueSet variable="tick-step">
      <value value="4500"/>
    </enumeratedValueSet>
  </experiment>
</experiments>
