...
; turtle-label-on? global variable used as slider
 ...
 ;; bear-related
  ...
  bci-thres ; sensitivity analysis
  ;bci-thres-males; sensitivity analysis - global switch
  ;bci-thres-females; sensitivity analysis - global switch
  ;forage-prob-below-bci; sensitivity analysis - global switch
  ;forage-prob-above-bci; sensitivity analysis - global switch
  ;forage-or-not-prob; sensitivity analysis - global switch
...
to setup-agents
  ...
  ; sensitivity analysis create file if doesn't exists
  if not (file-exists? (word "../output/sensitivity_results.csv"))
  [
    carefully
    [
      file-open (word "../output/sensitivity_results.csv")
      file-print csv:to-row (list "sensing-radius" "bci-thres-female" "bci-thres-male" "forage-prob-below-bci" "forage-prob-above-bci" "forage-or-not-prob" "initial-num-bears" "ticks" "bear-who" "bear-age" "bear-sex" "bear-age-class" "bear-spring-body-mass" "bear-body-mass" "bear-survival-rates" "bear-survive" "bear-road-freq" "bear-road-dens" "bear-total-dist" "bear-current-bci" "bear-cub" "bear-yearling")
      file-close
    ]
    [print error-message]
  ]
...
to check-stop-for-forage
  if (sex = "male") [set bci-thres bci-thres-males]
  if (sex = "female") [set bci-thres bci-thres-females]

  ifelse (diff-bci < bci-thres)
  [
    ifelse (random-float 1 < forage-prob-below-bci)
    [set should-I-forage? TRUE]
    [set should-I-forage? FALSE]
  ]
  [
    ifelse (random-float 1 < forage-prob-above-bci) [
...
to forage
  ...
  ifelse (random-float 1.0 < forage-or-not-prob)
  [
...
to record-results
  ...
  ;bear-dig-energy
    ...
    ; sensitivity results
    file-open (word "../output/sensitivity_results.csv")
    file-print csv:to-row (list sensing-radius bci-thres-females bci-thres-males forage-prob-below-bci forage-prob-above-bci forage-or-not-prob initial-num-bears ticks bear-who bear-age bear-sex bear-age-class bear-spring-body-mass bear-body-mass bear-survival-rates bear-survive bear-road-freq bear-road-dens bear-total-dist bear-current-bci bear-cub bear-yearling)
    file-close
  ]
end
...
to record-location
  ...
  file-open (word "../output/location_results_" behaviorspace-run-number ".csv")
  file-print csv:to-row (list behaviorspace-run-number ticks location-who location-xcor location-ycor)
  file-close
  ]
end
...
to-report mean-radius-energy
  ...
  let c sum [available-patch-energy] of b / count b ; average weighted-patch-energy over 13 patches if radius is 2
  report c
end
...
@#$#@#$#@
<experiments>
  <experiment name="sensitivity_analysis" repetitions="1" runMetricsEveryStep="false">
...
initialize-bear-param</setup>
    ...
    <enumeratedValueSet variable="bci-thres-females">
      <value value="0"/>
      <value value="1.5"/>
      <value value="-2"/>
      <value value="2"/>
    ...
    <enumeratedValueSet variable="bci-thres-males">
      <value value="1.5"/>
      <value value="-1"/>
      <value value="-1.5"/>
      <value value="0"/>
    ...
    <enumeratedValueSet variable="forage-prob-pos-bci">
      <value value="0.85"/>
      <value value="0.95"/>
      <value value="0.8"/>
      <value value="0.95"/>
    ...
    <enumeratedValueSet variable="forage-prob-neg-bci">
      <value value="0.75"/>
      <value value="0.65"/>
      <value value="0.5"/>
      <value value="0.8"/>
